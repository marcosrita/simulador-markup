...
# Estilos e controle dos menus por responsividade
st.markdown("""
    <style>
...
if pagina == "Início":
    ...
    if not st.session_state['produtos']:
        st.info("📌 Cadastre produtos antes de gerar os gráficos.")
    else:
        df = pd.DataFrame(st.session_state['produtos'])
        fig = px.bar(df, x="Produto", y="Lucro", text_auto=True)
        st.plotly_chart(fig)
    ...
    if st.button("Salvar Dados como CSV"):
        pd.DataFrame(st.session_state['produtos']).to_csv("dados_produtos.csv", index=False)
        st.success("Arquivo CSV salvo como dados_produtos.csv")
    arquivo = st.file_uploader("Carregar Arquivo CSV", type="csv")
    if arquivo:
        df = pd.read_csv(arquivo)
        st.session_state['produtos'] = df.to_dict(orient='records')
